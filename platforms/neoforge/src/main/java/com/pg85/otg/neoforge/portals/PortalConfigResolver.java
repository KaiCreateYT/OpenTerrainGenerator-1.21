...
/**
 * NeoForge-specific portal configuration resolver.
 * Delegates platform-agnostic logic to PortalConfigLookup,
 * handles NeoForge-specific material conversion.
 */
public final class PortalConfigResolver {
     ...
     * NeoForge-specific: converts LocalMaterialData to NeoForge BlockState.
     */
    public static boolean isFrameBlock(Block block, List<LocalMaterialData> frameBlocks) {
        ...
        for (LocalMaterialData material : frameBlocks) {
            if (((NeoForgeMaterialData) material).getState().getBlock() == block) {
                return true;
            }
        }
        return false;
     ...
     * NeoForge-specific: returns NeoForge BlockState.
     */
    public static BlockState getFrameBlock(ServerLevel level, String portalColor) {
        ...
        if (level.getChunkSource().getGenerator() instanceof OTGNeoForgeChunkGenerator gen) {
            ...
            if (portalBlocks != null && !portalBlocks.isEmpty()) {
                return ((NeoForgeMaterialData) portalBlocks.get(0)).getState();
            }
        }
        ...
        return findSettingsByColor(portalColor)
                ...
                .map(s -> ((NeoForgeMaterialData) s.getPortalBlocks().get(0)).getState())
