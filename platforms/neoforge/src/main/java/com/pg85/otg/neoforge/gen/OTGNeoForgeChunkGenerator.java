...
public class OTGNeoForgeChunkGenerator extends ChunkGenerator {
    ...
    public OTGNeoForgeChunkGenerator(
            OTGNeoForgeBiomeProvider biomeSource, Holder<NoiseGeneratorSettings> settings, Registry<Biome> biomeHolderGetter
    ) {
        super(biomeSource);
    ...
    public void applyBiomeDecoration(WorldGenLevel worldGenLevel, ChunkAccess chunkAccess, StructureManager structureManager) {
        ...
        NeoForgeWorldGenRegion fabricWorldGenRegion = new NeoForgeWorldGenRegion(this.preset.getFolderName(), OTG.getEngine().getPluginConfig(), this.preset.getPresetConfig(), otgWorldInfo, worldGenLevel, chunkAccess, this);
    ...
    private void handleOTGCarvers(long seed, ChunkAccess chunkAccess, GenerationStep.Carving carving) {
        NeoForgeBiome biome = (NeoForgeBiome) this.internalGenerator.getCachedBiomeProvider().getNoiseBiome(chunkAccess.getPos().x << 2, chunkAccess.getPos().z << 2);
        ...
        ChunkBuffer chunkBuffer = new NeoForgeChunkBuffer(chunkAccess);
        CarvingMask carvingMask = ((ProtoChunk) chunkAccess).getOrCreateCarvingMask(carving);
    ...
    public CompletableFuture<ChunkAccess> fillFromNoise(
        ...
        ChunkBuffer buffer = new NeoForgeChunkBuffer(chunkAccess);
    ...
    private int sampleHeightmap(int x, int z, @Nullable BlockState[] blockStates, @Nullable Predicate<BlockState> predicate)
    {
        ...
        for (int noiseY = this.internalGenerator.getNoiseSizeY() - 1; noiseY >= 0; --noiseY)
        {
            ...
            for (int pieceY = 7; pieceY >= 0; --pieceY)
            {
                ...
                // y is a 0-based offset from bottom of world; convert to world Y for getBlockState
                ...
                // return world Y coordinate (offset + minY + 1 for heightmap convention)
